...
pub const OUTPUT_SCHEMA_FIRST_SEARCH: &str = r#"{
  ...
  "properties": {
    ...
    "start_date": {"type": "string", "description": "開始日、フォーマットYYYY-MM-DD、search_news_by_dateツールのみ必要"},
    "end_date": {"type": "string", "description": "終了日、フォーマットYYYY-MM-DD、search_news_by_dateツールのみ必要"}
  },
...
pub fn system_prompt_report_structure() -> String {
    format!(
        r#"你是一位深度研究助手。给定一个查询，你需要规划一个报告的结构和其中包含的段落。最多五个段落。
确保段落的排序合理有序。
...
pub fn system_prompt_first_summary() -> String {
    format!(
        r#"你是一位专业的新闻分析师和深度内容创作专家。你将获得搜索查询、搜索结果以及你正在研究的报告段落，数据将按照以下JSON模式定义提供：
...
pub fn system_prompt_report_formatting() -> String {
    format!(
        r#"你是一位资深的新闻分析专家和调查报告编辑。你专精于将复杂的新闻信息整合为客观、严谨的专业分析报告。
你将获得以下JSON格式的数据：
