//! 検索ノード実装 (InsightEngine 版)
//!
//! DB ツール対応の検索クエリとリフレクションクエリの生成。
...
fn process_search_output(output: &str, default_query: &str, default_reasoning: &str) -> Value {
    ...
    // InsightEngine 固有フィールド
    if let Some(platform) = result.get("platform").and_then(|v| v.as_str()) {
        output_obj["platform"] = Value::String(platform.to_string());
    }
    if let Some(tp) = result.get("time_period").and_then(|v| v.as_str()) {
        output_obj["time_period"] = Value::String(tp.to_string());
    }
    if let Some(es) = result.get("enable_sentiment") {
        output_obj["enable_sentiment"] = es.clone();
    }
...
impl Node for ReflectionNode {
    ...
    async fn run(&self, input_data: &Value) -> Result<Value> {
        if !self.validate_input(input_data) {
            anyhow::bail!(
                "入力データのフォーマットエラー、title、content、paragraph_latest_state フィールドが必要"
            );
        }
